...
const frontendOrigin = process.env.FRONTEND_URL; // Render provides this variable
...
app.get('/api/analytics/results-summary', auth, authorizeRoles('medical_staff', 'admin', 'doctor', 'financial_admin'), async (req, res) => {
    try {
        ...
        const finalResults = await pool.query('SELECT COUNT(*) AS final_results FROM patient_results_files WHERE result_status = \'final\'');
