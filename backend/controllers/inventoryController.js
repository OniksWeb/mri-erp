...
export const deleteItem = async (req, res) => {
    try {
        const { id } = req.params;
        console.log(`Attempting to delete item with ID: ${id} by user:`, req.user);

        const result = await pool.query('DELETE FROM inventory_items WHERE id = $1 RETURNING *', [id]);
        
        if (result.rows.length === 0) {
            console.log(`Delete failed: Item with ID ${id} not found.`);
            return res.status(404).json({ error: 'Item not found in database' });
        }

        console.log(`Successfully deleted item:`, result.rows[0]);
